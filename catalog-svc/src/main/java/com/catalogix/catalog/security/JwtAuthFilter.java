...
/**
 * Validates the "Authorization: Bearer <token>" header on every product-svc
 * request except health/metrics. Replaces the old dev-only "trust the
 * X-USER-ID header the client sends" approach — the caller's identity now
 * comes only from a signature-verified token minted by user-svc.
 */
@Component
@Order(2)
