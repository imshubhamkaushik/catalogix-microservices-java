    ...
    public class ProductRepositoryIntegrationTest {
        ...
        void saveAndFindProduct() {
            Product p = new Product(LAPTOP_NAME, "A fast laptop", new BigDecimal("55000.00"));
            p.setOwnerId(1L);
            Product saved = productRepository.save(p);

            Long savedId = Objects.requireNonNull(saved.getId(), "saved ID must not be null");
            assertThat(saved.getId()).isNotNull();
            assertThat(saved.getName()).isEqualTo(LAPTOP_NAME);
            ...
            assertThat(found.get().getName()).isEqualTo(LAPTOP_NAME);
        }
