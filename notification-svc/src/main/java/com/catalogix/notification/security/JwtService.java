...
/**
 * Verifies JWTs — issued either by user-svc (for a real user's session) or
 * minted as a short-lived system token by user-svc/order-svc when they need
 * to call notification-svc outside of any user request (e.g. a scheduled
 * retry, or a password-reset email sent before the user has any session at
 * all). notification-svc never issues its own tokens.
 *
