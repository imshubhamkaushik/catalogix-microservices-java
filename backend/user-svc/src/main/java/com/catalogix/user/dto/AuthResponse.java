...
// on every request. refreshToken is the long-lived opaque token — it is
// intentionally @JsonIgnore'd below and NEVER appears in the JSON body;
// UserController reads it here in Java only to set it as an httpOnly
// Set-Cookie header. Keeping a long-lived credential out of JS-readable
// response bodies (and therefore out of localStorage/sessionStorage, wherever
// the frontend stores the rest of this response) means an XSS bug in the SPA
// can no longer exfiltrate a portable, long-lived credential — at worst it
// can abuse the live session while a tab is open, not carry it away.
